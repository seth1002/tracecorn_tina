...
class Unitracer(object):
    ...
    def getstrw(self, addr, size=100):
        data = ""
        for i in range(0, size, 2):
            data += self.emu.mem_read(addr+i, 2)
            if data.endswith('\x00\x00'):
                break
        # remove last 2 zero bytes
        return unicode(bytes(data[:-2]), 'U16')

    def ucreg(self, n):
        return self.regmap[n][self.is64]


    def dump_mem(self, addr, size = 100):    
        bData = self.emu.mem_read(addr, size)
        dump_data = ['{:02x}'.format(x) for x in bData]
    ...
    def dumpregs(self, regs):
        ...
        for reg in regs:
            ...
            fmt += (" {0}: 0x{1:0"+str(self.bytes*2)+"x} ").format(reg, val)
            if idx%4 == 0:
                fmt += '\n'
            idx +=1
        print(fmt)
