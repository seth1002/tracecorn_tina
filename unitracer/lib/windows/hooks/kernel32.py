...
def GetCurrentThreadId(ut):
    ...
    print('GetCurrentThreadId() = {} => 0x{:08x}'.format(ut.current_thread_id, retaddr))
    # print(hex(retaddr))
    ut.emu.reg_write(UC_X86_REG_EAX, ut.current_thread_id)
    ut.pushstack(retaddr)


def GetCurrentProcess(ut):
    retaddr = ut.popstack()
    
    print('GetCurrentProcess() = 0x{:08x} => 0x{:08x}'.format(ut.ADDRESS, retaddr))
    ut.emu.reg_write(UC_X86_REG_EAX, ut.current_thread_id)
    ut.pushstack(retaddr)
...
def FlsSetValue(ut):
    ...
    lpFlsData = ut.popstack()
    res = 1

    ut.fls_stors[dwFlsIndex] = lpFlsData
    print('FlsSetValue(0x{:08x}, 0x{:08x}) = 0x{:08x} => 0x{:08x}'.format(
        dwFlsIndex, lpFlsData, res, retaddr
    ))
    ut.emu.reg_write(UC_X86_REG_EAX, res)
    ut.pushstack(retaddr)
...
def ExitProcess(ut):
    ...
    print 'ExitProcess ({}) => 0x{:08x}'.format(uExitCode, retaddr)
    ut.emu.emu_stop()
    ut.pushstack(retaddr)
...
def IsDebuggerPresent(ut):
    ...
    print 'IsDebuggerPresent() = {} => 0x{:08x}'.format(res, retaddr)
    ut.emu.reg_write(UC_X86_REG_EAX, res)
    ut.pushstack(retaddr)
...
def GetModuleHandleW(ut):
    ...
    print('GetModuleHandleW(lpModuleName=0x{:08x}:"{}") = 0x{:08x} => 0x{:08x}'.format(lpModuleName, ModuleName, hModule, retaddr))
...
def GetModuleHandleA(ut):
    ...
    print('GetModuleHandleA(lpModuleName=0x{:08x}:"{}") = 0x{:08x} => 0x{:08x}'.format(lpModuleName, ModuleName, hModule, retaddr))
    ut.emu.reg_write(UC_X86_REG_EAX, hModule)
    ut.pushstack(retaddr)
...
def LoadLibraryW(ut):
    ...
    print('LoadLibraryW (lpFileName="{0}") = {1:08x} => 0x{2:08x}'.format(lpFileName_s, res, retaddr))
...
def VirtualProtect(ut):
    ...
    res = 1
    print('VirtualProtect (lpAddress={:08x}, dwSize={:08x}, flNewProtect={:08x}, lpflOldProtect={:08x}) = {} => 0x{:08x}'.format(
        lpAddress, dwSize, flNewProtect, lpflOldProtect, res, retaddr
    ))
...
def GetModuleFileNameW(ut):
    ...
    ut.emu.mem_write(lpFilename, ut.command_line.encode('UTF-16LE'))

    _str = ut.getstrw(lpFilename)
    strlen = len(_str)
 
    res = strlen
    print('GetModuleFileNameW(0x{:08x}, lpString=0x{:08x} -> "{}", 0x{:08x}) = {:08x} => 0x{:08x}'.format(hModule, lpFilename, _str, nSize, strlen, retaddr))
...
def FindResourceA(ut):
    ...
    if lpType not in rt_rcdata_idx_list:
        ...
        rt_rcdata_idx = rt_rcdata_idx_list.index(lpType)
...
def GetNativeSystemInfo(ut):
    retaddr = ut.popstack()
    lpSystemInfo = ut.popstack()

    res = 0
    system_info = win_os_cfg.gen_system_ifno()
    ut.emu.mem_write(lpSystemInfo, system_info)
    print('GetNativeSystemInfo(0x{:08x}) = {} => 0x{:08x}'.format(lpSystemInfo, res, retaddr))
