...
class Windows(Unitracer):
    ...
    def _init_ldr(self, dlls=None, exe_ldr=None):
        ...
        for dll_name in dlls:
            dll_name = dll_name.upper().split('.')[0]
            if dll_name in m_hooks.hookdlls:
                if hasattr(m_hooks.hookdlls[dll_name], 'init_dll'):
                    init_dll = getattr(m_hooks.hookdlls[dll_name], 'init_dll')
                    if init_dll:
                        init_dll(self)
    ...
    def dump_heap(self, dump_path):
        ...
        for addr_range, mem_info  in self.alloc_mem_range_list.items():
            try:
                ...
                file_path = dump_path+'/'+addr_range 
                print('dump hep:{}'.format(file_path))
    ...
    def start(self, offset):
        ...
        try:
            ...
            # befor start, push some trash data
            self.pushstack(0)
            self.pushstack(0)
            self.pushstack(0)
